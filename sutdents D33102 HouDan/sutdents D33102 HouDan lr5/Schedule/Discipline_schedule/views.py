...
def teacher_list(response):
    ...
    contex = locals()
    return render(response, "teacher_list2.html", context=contex)
...
def schedule_list(response,idgroup):
    cl=Class.objects.get(pk=idgroup)
    ...
    return render(response, "schedule_list2.html", context=locals())

def update_schedule_list(response,idgroup):
    cl=Class.objects.get(pk=idgroup)
    Schdules = Schedule.objects.filter(id_group=idgroup)
    Monday = Schdules.filter(date__week_day=1)
    Tuesday = Schdules.filter(date__week_day=2)
    ...
    Saturday = Schdules.filter(date__week_day=6)
    Sunday = Schdules.filter(date__week_day=0)
    return render(response,"update_schedule_list.html", context=locals())

def delete_schedule_deta(respons,pk):
    idgroup=Schedule.objects.get(pk=pk).id_group.id
    sch = Schedule.objects.get(pk=pk)
    sch.delete()
    return HttpResponseRedirect("/D-S/schedule_list/%d/"%(idgroup))
